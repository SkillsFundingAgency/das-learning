...
public class UpdateLearnerStepDefinitions
{
    ...
    public async Task WhenTheUpdateRequestIsSent()
    {
        ...
        var updateResponse =  await _testContext.TestInnerApi.Put<UpdateLearnerRequest, UpdateLearnerResult>($"/{Constants.UkPrn}/{learnerKey}", updateRequest);
    ...
    public async Task ThenTheFollowingEnglishAndMathsDetailsAreStored(Table table)
    {
        ...
        foreach (var row in table.Rows)
        {
            var pauseDate = row.ContainsKey("PauseDate") && !string.IsNullOrWhiteSpace(row["PauseDate"])
